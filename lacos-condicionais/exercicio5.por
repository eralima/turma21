programa
{
	...
	funcao inicio()
	{
		 ...
		 * Faça um sistema que leia o índice de poluição medido e emita a notificação adequada aos diferentes grupos de empresas.*/
		 ...
		 //processamento
		 se(indice <= 0.25){
		 	escreva("Indústrias continuem suas atividades")
		 	}
		 senao se(indice > 0.25 e indice < 0.4){
		 	escreva("Indústrias do grupo 1º paralisem suas atividades")
		 }
		 senao se (indice >= 0.4 e indice < 0.50){
		 	escreva("Indústrias do grupo 1º e 2º grupo paralisem suas atividades")
		 }
